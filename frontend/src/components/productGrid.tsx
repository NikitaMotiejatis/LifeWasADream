    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {products.map(p => (
          <button
            key={p.name}
            className="group relative flex flex-col items-center rounded-lg bg-white p-4 text-center shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md active:scale-95"
          >
            <div className="mb-3 aspect-square w-full max-w-24 rounded-lg bg-gray-200 shadow-inner" />

            <div className="w-full px-2">
              <p
                className="line-clamp-2 text-center text-sm leading-snug font-medium wrap-anywhere hyphens-auto group-hover:line-clamp-none"
                title={p.name}
              >
                {p.name}
              </p>
